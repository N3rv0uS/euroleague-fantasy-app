...
def unify_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Προσπαθεί να χαρτογραφήσει BN headers στα standard μας.
    Καλύπτει συνηθισμένες παραλλαγές: '2P M/A', '2PM-A', '2PTS', '2-pt made/att', κ.λπ.
    """
    # normalize headers
    ...
    alias: Dict[str, List[str]] = {
        ...
        # M/A τύποι + ποσοστά (κρατάμε το M/A στο κύριο, το % δεν μας χρειάζεται για split)
        ...
        "T":    ["TRB","Tot Reb","REB","Total Rebounds","T"],
        "AST":  ["AST","Assists","A"],
        "STL":  ["STL","Steals","S"],
        "TOV":  ["TOV","TO","Turnovers"],
        "BLK_FV":["BLK","Blocks For","Blocks Made","Blocks"],
        "BLK_AG":["BLK Against","Blocks Against","BA","Blocks Received"],
        "FLS_CM":["Fouls","Fouls Committed","PF","Cm","Personal Fouls"],
        "FLS_RV":["Fouls Drawn","FD","Rv","Fouls Received"],
        "PIR":  ["PIR","EFF","Index","Performance Index Rating","Efficiency"],
        # Συχνές στήλες αγώνα (κρατάμε για info)
        "W":    ["W","Win"],
        "L":    ["L","Loss"],
        "Team": ["Team","Opponent","Opp."],
        "Team.1":["Team.1","Opponent.1","Opp.1"],
        "game_no":["Game","#","No","Match #"],
    }

    # phase 1: direct rename if exact match
    rename = {}
    ...
    # phase 2: fuzzy contains (π.χ. "2P M/A %")
    for std, keys in alias.items():
    ...
    # Διαχωρισμός made/attempted για 2FG/3FG/FT (όπου η στήλη έχει μορφή "x/y")
    ...
    # standard columns to ensure presence
    standard = ["MIN","PTS","2FG","3FG","FT","2FG_M","2FG_A","3FG_M","3FG_A","FT_M","FT_A",
                "O","D","T","AST","STL","TOV","BLK_FV","BLK_AG","FLS_CM","FLS_RV","PIR",
                "W","L","Team","Team.1","game_no"]
    for col in standard:
...
def parse_bn_html_gamelogs(page_html: str) -> pd.DataFrame:
    """
    Προσπαθεί να εντοπίσει τον πίνακα 'Statistics: Games' στη σελίδα BN.
    Κρατά τον μεγαλύτερο σχετικό πίνακα (με βάση το πόσες στήλες-στόχους ταιριάζουν).
    """
    tables = pd.read_html(page_html)
    if not tables:
        return pd.DataFrame()

    wanted_tokens = {"MIN","PTS","2P","3P","FT","REB","AST","STL","TO","EFF","PIR"}
    best = None
    best_score = -1
    for t in tables:
        t = flatten_headers(t)
